...
class TreeLayoutCalculator {
	...
	public static Map<Node, Point> calculateLayout(Map<Point, Node> pointToNodeMap, List<Edge> edges, int panelWidth, int panelHeight, Node specifiedRoot) {
		...
		double currentX = 450.0;
		for (Node root : roots) {
			Map<Node, List<Node>> tree = buildTreeFromGraph(root, adjacencyList, visited);

			// 優化: 計算每個子樹的寬度，用以精確分配水平位置
			Map<Node, Double> subtreeWidths = new HashMap<>();
			calculateSubtreeWidths(root, tree, subtreeWidths);

			// 優化: 調整位置分配邏輯
			assignPositions(root, tree, layoutMap, currentX, 50, subtreeWidths);

			// 優化: 根據根節點的總寬度來計算下一個根節點的起始位置
			currentX += subtreeWidths.getOrDefault(root, 100.0) + 50.0;
		}
		return layoutMap;
	}

	private static void assignPositions(Node node, Map<Node, List<Node>> tree, Map<Node, Point> layoutMap, double x, double y, Map<Node, Double> subtreeWidths) {
		...
		// 優化: 計算子節點的總寬度，並依此分配水平起始位置
		double childrenTotalWidth = children.stream().mapToDouble(c -> subtreeWidths.getOrDefault(c, 1.0)).sum();
		double currentChildX = x - (childrenTotalWidth / 2.0);

		for (Node child : children) {
			double childWidth = subtreeWidths.getOrDefault(child, 1.0);
			double childX = currentChildX + (childWidth / 2.0);
			assignPositions(child, tree, layoutMap, childX, y + 50, subtreeWidths);
	...
	private static void calculateSubtreeWidths(Node node, Map<Node, List<Node>> tree, Map<Node, Double> widths) {
		List<Node> children = tree.getOrDefault(node, new ArrayList<>());
		if (children.isEmpty()) {
			// 這裡將基礎寬度從 100.0 改為 50.0 或更小
			widths.put(node, 50.0); // 最小節點寬度，可以調整
			return;
		}
