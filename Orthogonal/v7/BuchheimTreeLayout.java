...
class BuchheimTreeLayout {
	...
	/**
	 * 根據 Buchheim 演算法計算節點的 (x, y) 座標，並在水平與垂直方向保持固定間距
	 */
	public static Map<Node, Point> calculateLayout(Map<Point, Node> pointToNodeMap, List<Edge> edges, int panelWidth, int panelHeight, Node specifiedRoot) {
		...
		double currentX = 100;
		double nodeWidth = 10; // 節點顯示寬度
		double horizontalSpacing = 40; // 節點之間的最小水平間隔
		double verticalSpacing = 70; // 每層節點之間的垂直間隔
		...
		for (Node root : roots) {
			...
			firstWalk(root, tree, nodeDataMap, nodeWidth, horizontalSpacing);
	...
	/**
	 * 第一遍遍歷：計算每個節點的 初始x座標 和 子樹的輪廓 引入了 nodeWidth 和 horizontalSpacing 參數
	 */
	private static void firstWalk(Node node, Map<Node, List<Node>> tree, Map<Node, NodeData> nodeDataMap, double nodeWidth, double horizontalSpacing) {
		...
		if (children.isEmpty()) {
			data.x = 0;
			data.subtreeWidth = nodeWidth + horizontalSpacing;
			return;
		}

		Node previousChild = null;
		for (Node child : children) {
			firstWalk(child, tree, nodeDataMap, nodeWidth, horizontalSpacing);
			data.subtreeWidth += nodeDataMap.get(child).subtreeWidth;

			if (previousChild != null) {
				// 修正：使用 getShift 函數來處理子樹合併和固定間隔
				double shift = getShift(child, previousChild, tree, nodeDataMap, horizontalSpacing);
	...
	/**
	 * 計算合併兩個相鄰子樹所需的最小平移量，並強制保持 horizontalSpacing
	 */
	private static double getShift(Node right, Node left, Map<Node, List<Node>> tree, Map<Node, NodeData> nodeDataMap, double horizontalSpacing) {
		double shift = 0;
		...
		while (!leftQueue.isEmpty() && !rightQueue.isEmpty()) {
			...
			// 計算所需的偏移量，確保有 horizontalSpacing 的間隔
			double neededShift = (leftData.x + leftData.subtreeWidth) + horizontalSpacing - rightData.x;
	...
	private static void secondWalk(Node node, Map<Node, List<Node>> tree, Map<Node, NodeData> nodeDataMap, Map<Node, Point> layoutMap, double x, double y) {
		...
		for (Node child : tree.getOrDefault(node, Collections.emptyList())) {
			secondWalk(child, tree, nodeDataMap, layoutMap, x + data.mod, y + 70); // 這裡調整垂直間距
		}
	}
