...
//		double totalGraphWidth = 0;
//		for (Node root : roots) {
//			Map<Node, List<Node>> tree = buildTreeFromGraph(root, adjacencyList, visited);
//			totalGraphWidth += subtreeWidth(root, tree);
//		}
		...
		for (Node root : roots) {
			...
			int subtreeTotalWidth = subtreeWidths.getOrDefault(root, 1);
			double scaleFactor = (panelWidth / (double) roots.size()) / subtreeTotalWidth;
			Map<Node, Double> xPositions = new HashMap<>();
			assignPositions(root, tree, layoutMap, currentX, 50, scaleFactor, xPositions);

			currentX += subtreeWidth(root, tree) * horizontalSpacing + 50;
		}

		return layoutMap;
	}

	private static Map<Node, List<Node>> buildTreeFromGraph(Node root, Map<Node, List<Node>> adjacencyList, Set<Node> visitedNodes) {
		...
		while (!queue.isEmpty()) {
			...
			for (Node child : adjacencyList.getOrDefault(parent, Collections.emptyList())) {
				if (visitedNodes.contains(child) && !visited.contains(child)) {
					visited.add(child);
	...
	private static void calculateSubtreeWidths(Node node, Map<Node, List<Node>> tree, Map<Node, Integer> widths) {
		...
		for (Node child : children) {
			...
			totalWidth += widths.getOrDefault(child, 1);
		}
		widths.put(node, totalWidth);
	}

	private static void assignPositions(Node node, Map<Node, List<Node>> tree, Map<Node, Point> layoutMap, double parentX, double y, double scaleFactor, Map<Node, Double> currentXPositions) {
		layoutMap.put(node, new Point(parentX, y));

		double childX = parentX - (subtreeWidth(node, tree) / 2.0) * scaleFactor;

		for (Node child : tree.getOrDefault(node, new ArrayList<>())) {
			double childSubtreeWidth = subtreeWidth(child, tree);
			double newChildX = childX + (childSubtreeWidth / 2.0) * scaleFactor;

			assignPositions(child, tree, layoutMap, newChildX, y + 30, scaleFactor, currentXPositions); // 100
			childX += childSubtreeWidth * scaleFactor;
		}
	}

	private static int subtreeWidth(Node node, Map<Node, List<Node>> tree) {
		List<Node> children = tree.getOrDefault(node, new ArrayList<>());
		if (children.isEmpty()) {
			return 1;
		}
...
public class OrthogonalTreeGUIv5 {
