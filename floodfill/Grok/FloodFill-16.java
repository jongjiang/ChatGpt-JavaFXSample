public class FloodFill {
    public void floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        int maxSize = Math.max(8, (rows * cols) >> 2); // Tighter size estimate
        int[] queue = new int[maxSize << 1]; // Interleaved row, col
        int front = 0, rear = 0;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0, len = image.length * image[0].length, w = image[0].length; i < len; i++) {
            sb.append((char) (image[i / w][i % w] + '0')).append(i % w < w - 1 ? ' ' : i < len - 1 ? '\n' : "");
        }
        System.out.print(sb);
    }
}